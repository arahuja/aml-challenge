...
from scorers import normalized_pcc_metric
...
def predict_remission(X, y, X_test,
    ...
    return build_model(model, X, y, X_test, output)
...
def predict_remission_length(remission_model, X, y, X_test,
    ...
    model = lr_model if linear else rf_model
    return build_model(model, X, y, X_test, output, scorer = normalized_pcc_metric)
